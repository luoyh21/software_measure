...
def _show_plot_inline(plot_dir: Path) -> None:
    """Best-effort inline image preview using whichever tool is installed."""
    ...
    console.print(Panel(
        ...
        title="afl-plot images",
        border_style="green",
    ))
    viewer = None
    for cand in ("chafa", "viu", "tiv"):
        if shutil.which(cand):
            viewer = cand
            break
    if viewer is None:
        console.print(
            "[dim]Tip: install [bold]chafa[/] (apt install chafa) to preview "
            "the coverage chart inline. PNGs are saved at the path above; "
            "open them in any image viewer for screenshots.[/]"
        )
        return
    for p in pngs:
        console.print(f"\n[bold cyan]── {p.name} ──[/]")
